...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    loss,grad_2=L2_loss(scores,y)
    loss=loss + self.reg*0.5*np.sum(W1*W1) + self.reg*0.5*np.sum(W2*W2)

    gradx_2,gradw_2,gradb_2=affine_backward(grad_2,cache_2)
    _,gradw_1,gradb_1=affine_relu_backward(gradx_2,cache_1)


    grads['W1'],grads['W2'],grads['b1'],grads['b2']=gradw_1+self.reg*W1,gradw_2+self.reg*W2,gradb_1,gradb_2
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(self.num_layers-2):
      ...
      out_1,cache_1=affine_relu_forward(out[0],W,b)
      out.pop()
      out.append(out_1)
      cache.append(cache_1)
    W,b=self.params['W'+str(self.num_layers)],self.params['b'+str(self.num_layers)] 
    scores,cache_1=affine_forward(out[0],W,b)
    cache.append(cache_1)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    # of 0.5 to simplify the expression for the gradient.                      #
    ############################################################################
    loss,grad_2=softmax_loss(scores,y)
    gradx=[]
    for i in xrange(1,self.num_layers+1):
      W=self.params['W'+str(i)]
      loss=loss + self.reg*0.5*np.sum(W*W)
    cache_1=cache.pop()
    grad_x,dW,grads['b'+str(self.num_layers)]=affine_backward(grad_2,cache_1)
    grads['W'+str(self.num_layers)]=dW+self.reg*self.params['W'+str(self.num_layers)]
    gradx.append(grad_x)
    for i in xrange(1,self.num_layers):
      cache_1=cache.pop()
      grad_x,dW,grads['b'+str(self.num_layers-i)]=affine_relu_backward(gradx[0],cache_1)
